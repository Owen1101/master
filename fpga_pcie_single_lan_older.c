...
//	flag = 0;
	...
	dma_sync_single_for_cpu(&adapter->pdev->dev,read_phyaddr,
							/*BUF_SIZE*/count,PCI_DMA_FROMDEVICE);//刷新cache
//	dma_sync_single_for_cpu(&adapter->pdev->dev,read_phyaddr,
//							BUF_SIZE,PCI_DMA_FROMDEVICE);//刷新cache
//	dma_sync_single_for_device(&adapter->pdev->dev,read_phyaddr,
//							BUF_SIZE,PCI_DMA_TODEVICE);//刷新cache
	result = copy_to_user(buf,read_buf,count);
//	printk("read_buf = %x\n",readl(read_buf));
//	printk("read_buf = %x\n",readl(read_buf));
//	printk("read_buf = %x\n",readl(read_buf + 0xfffff0));
//	printk("read_buf = %x\n",readl(read_buf + 0xfffff4));
//	printk("read_buf = %x\n",readl(read_buf + 0xfffff8));
//	printk("read_buf = %x\n",readl(read_buf + 0xfffffc));
	if(result < 0){
...
//	read_phyaddr = virt_to_phys(read_buf);
	read_phyaddr = virt_to_dma(&adapter->pdev->dev,read_buf);
//	read_phyaddr = virt_to_pfn(read_buf);
...
//	writel(0x4c4b400,adapter->pcie_bar0 + TRIGGER_CNT_EXTERN);
	...
	writel(0x1,adapter->pcie_bar0 + AD_TEST_SEL);
	...
	udelay(200);
	writel(0x20,adapter->pcie_bar0 + WORK_MODE);//工作模式
...
//		printk("Bar0 region request success.\n");
	...
	read_buf = kmalloc(BUF_SIZE,GFP_DMA);//申请DMA空间
//	read_buf = dma_alloc_coherent(&adapter->pdev->dev,BUF_SIZE,&read_phyaddr,GFP_DMA);
	if(NULL == read_buf){
